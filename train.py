...
MIN_SAMPLES_PER_CLASS = 100 #change a lot
BATCH_SIZE = 512
LEARNING_RATE = 1e-3
LR_STEP = 3
LR_FACTOR = 0.5
...
TIME_LIMIT = 9 * 60 * 60
img_size=(64,64)
path='../input/test-state-2/test/test/'
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.mode=='test':
            sample = Image.open(f'../input/test64/{filename}.jpg')
        else:
...
def load_data() -> 'Tuple[DataLoader[np.ndarray], DataLoader[np.ndarray], LabelEncoder, int]':
    ...
    test_df = pd.read_csv('../input/landmark-recognition-2019/recognition_sample_submission.csv', dtype=str)
    test_df.drop(columns='landmarks', inplace=True)
    print('test_df', test_df.shape)

    # filter non-existing test images
    exists = lambda img: os.path.exists(f'../input/test64/{img}.jpg')
